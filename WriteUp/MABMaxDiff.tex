...
With formal notation, we revisit the defining elements of the problem we study:
\begin{itemize}
	\item (finite budget $T << \infty$)
	\item (loss function, $l(a,y)$)
	\item (sequential data collection, $D \to D \cup (x,y) $)
	\item (pool of inputs, $x \in \mathcal{X}$)
...
For now, we consider the decision to be simply the full $\numitems$-item rank ordering, the most general form of ranking and selection problem. But one particular decision may be selecting the highest-utility item (top 1), the top set of items (top $\numtopset$, where $\numtopset << \numitems$), or a partial rank ordering of the top set of items. The decision could be judged rewarded or penalized based on more than just the ranking, but also the value of the items' utilities $u$. For instance, identifying the best item when it has much higher utility than the second-best may carry more weight for the manager than doing so when the top two items have nearly equal utilities. These will enter into the specification of the loss function, which is the error from ranking.
...
\begin{align} 
l(a,u) &= \frac{1}{k} \sum_{i \in 1:k} \left( 1 - a_k(i) \right) & \text{(binary)} \\
l(a,u) &= \frac{1}{k} \left( \sum_{i \in 1:k} u_{i}^{*} - \sum_{i = 1}^{K} a_k(i) u_{i}^{*} \right) & \text{(linear in utility)}\\
l(a,u) &= \frac{1}{k} \left( 

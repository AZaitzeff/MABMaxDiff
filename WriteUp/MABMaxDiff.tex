...
In ~\cite{toubia2007adaptive} a similar idea is used to avoid misclassifying an item without sampling it further. The author derived insight from genetic algorithms where ``decreasing the mutation rate over the number of iterations (or generations) leads to higher performance compared to a fixed mutation rate.'' This suggests sampling from a diffuse distribution gives benefit over sampling uniforming over the remaining items as the variance of the diffuse distributions shrink with the number of respondents interviewed. 

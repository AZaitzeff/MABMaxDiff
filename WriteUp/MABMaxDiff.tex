...
$\epsilon$-Diffuse TS closest to the threshold & Take the $(1-\epsilon)L$ and  $\epsilon L$ items that have sampled utility closest to $\frac{u_k+u_{k+1}}{2}$.\\
$\epsilon$-greedy closest to the threshold & Take the $(1-\epsilon)L$ items that have estimated utility closest to $\frac{u_k+u_{k+1}}{2}$ and $\epsilon L$ uniformly from the remaining.\\
...
The MaxDiff approaches we tested were the following. For our actual implementation of Bandit MaxDiff, we used Thompson Sampling to draw 20 of the 120 items for each respondent (tending to oversample the ``stars'' based on aggregate logit estimates from previous respondents).  As described earlier, we tested the different adaptive Maxdiff approaches found in table \ref{methods}, with ($\epsilon=\frac{1}{4}$, $\delta$=.25) for $\epsilon$-diffuse methods, against an existing fixed MaxDiff approach:\\
...
\label{table:nice500}
\end{table}
To test to see if these results would generalize to other data sets.

...
%
\newcommand{\fixedexpress}[1]{\textit{express}} 

% previously referred to as greedy
% we need to global change: greedy -> \egreedy
\newcommand{\egreedy}[1]{$\epsilon$-\textit{greedy}} 


% previously referred to as greedythres
% we need to global change: greedythres -> \egreedythres
\newcommand{\egreedythres}[1]{$\epsilon$-\textit{greedythres}} 

\newcommand{\mismin}[1]{\textit{max-misclass}} 

\newcommand{\ts}[1]{\textit{TS} } 

\newcommand{\edts}[1]{$\epsilon$-$\delta$-\textit{diffuse TS} } 

% previously referred to as TSregthres
\newcommand{\tsthres}[1]{\textit{TS-thres} } 

% previously referred to as TSthres
\newcommand{\edtsthres}[1]{$\epsilon$-$\delta$-\textit{thres} } 
\newcommand{\uncert}[1]{\textit{max-uncert} } 
...
One common managerial decision driving such data collection is to select which are most preferred among a large set of hundreds of possible items. For instance, consider these illustrative managerial decisions: \\
\begin{enumerate}
...
\textbf{The preceding paragraph needs to be changed. We are not doing any earning just learning Pure exploration}
...
\begin{figure}
\caption{Maximum Number of Items Studied via MaxDiff during 2015}
...
The $\epsilon$-$\delta$  generalization is just like a two-component mixture model, which nests a pooled model without segments. As the epsilon is zero, the diffuse distribution is never used, so the algorithm collapses to regular TS. As $\delta$ is 1, the diffuse distribution is equivalent to the non-diffuse distribution. For our empirical analysis we use ($\epsilon=\frac{1}{4}$, $\delta=\frac{1}{4}$). Also tested were ($\epsilon=\frac{1}{2}$, $\delta=\frac{1}{4}$) and ($\epsilon=1$, $\delta=\frac{1}{4}$) which performed on par with ($\epsilon=\frac{1}{4}$, $\delta=\frac{1}{4}$). Our recomended ranges for the parameters are $\frac{1}{5}\leq \epsilon \leq \frac{1}{2}$, $\frac{1}{4}\leq \delta \leq \frac{1}{2}$.\\

\subsubsection{Algorithm $\epsilon$-greedy}
As a baseline for the adaptive methods will we test the $\epsilon$-greedy.
Let $\theta$ be the current estimated parameters. Take the $(1-\epsilon)L$ items with the greatest $\theta$ value and choose the remaining $\epsilon L$ uniformly from the remaining items. For our empirical analysis we use $\epsilon=\frac{1}{4}$.
...
\textbf{Fixed Express MaxDiff}. We randomly drew $L=20$ of the $K=120$ items to show to each respondent.  Each item appeared $\frac{J*S}{L} = \frac{12*5}{20} = 3$ times per respondent.  Across respondents, each item also appeared the same number of times. 
...
\begin{figure}
\caption{Adaptive vs Static: \ts and \edts improve hit rate. \eric{Only show \fixedexpress, \egreedy, \ts, \edts here for k=3,10}}
...
However, we next examine the other proposed methods. Recall each algorithm selects $L=20$ items to serve to every respondent. But we examine hit rate for the top $k$ items, for $k = \{3,10,20,40\}$. 
...
But the differences among the other algorithms are more telling. We find that Thompson Sampling alone has its limits. It is not suited for this active learning problem, when there is little room for error, when we care about the hit rate for all items in the survey (the case $k=L=20$). While the \edts improves its performance for the other cases, that extra exploration does not direct resources toward the places that matter most: areas of high uncertainty at the decision boundary, not just near the top of the list and not scattered randomly.  

Overall, a smaller $k$ makes the problem more challenging to do well earlier. Measuring a hit rate for the top 3 is a higher bar than the hit rate for 40.  

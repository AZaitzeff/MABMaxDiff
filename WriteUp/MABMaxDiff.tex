...
% Quick outline:
% P&G and GM have a common problem. 
% Classic Idea Screening. Active learning / bandit w Adaptive MaxDiff. 
% New kind of problem. 
% This is how many people are trying to solve this problem. 
% 
% This isn't conjoint. 
% This needs to be adaptive.
% This would cost much more money. 
...
\begin{figure}
\caption{How \edts varies with $\epsilon$ and $\delta$. \eric{Show typical plot of hit rate over respondents. Show many \edts versions: eps = 1/2 (left) and eps = 1/4 (right), with multiple lines in each plot, delta = 1/2 (one color) and delta = 1/4 (another color). And in each of the plots also show regular \ts (eps=0,delta=1).} \eric{Add in Fixed Express, (eps=1,delta=0). } }

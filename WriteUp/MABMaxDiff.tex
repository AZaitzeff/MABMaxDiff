...
The problem we address appears across a range of marketing settings. Consider these illustrative managerial decisions with a common challenge to identify the best set of items out of many:
...
As this problem differs from the standard bandit setting, we make new methodological contributions. We introduce and test several new components to these algorithms. In particular, first, we introduce researcher-chosen parameters to control robustness to a changing environment when using Thompson Sampling. Our approach -- with the survey context in mind – differs because it prevents the case where the earliest respondents preferences, if they are not representative of the target population, from having an outsized influence on the questions for later respondents, hence, preserving the desired effectiveness. Second, we allow for the algorithms to specifically learn an unranked (or ranked) top set. And third, we evaluate a set of approximations to posterior sampling to allow for faster model updates. These three algorithmic innovaitons could also be applied apply outside of our idea screening and preference measurement problem.
...
CAPELLE ET AL: The ELO framework algorithms select the most informative examples to optimize DCG loss. The examples are chosen because, if predicted incorrectly, they may lead to a large DCG loss. These are the ones that the current model is most uncertain about.

\section{General Model}

We frame this as a sequential decision making problem under uncertainty where the decision maker (market researcher) selects a set of items to serve to the next respondent. 
...
The market researcher considers $n$ items with unknown true utilities $u_1, \ldots, u_\numitems$. Their unknown rank ordering, $u_{\pi^{*}(1)} > \ldots, u_{\pi^{*}(\numitems)}$. Uncertain about $u$ and $\pi^{*}$, she will have to make a decision based on her beliefs. The most general decision here is a permutation $\pi$ of length $\numitems$, where the rank of the $j$-th item is $\pi(j)$.  Each period she can survey customers to gain information about $\numperset$ items. She selects test items $X_q = \{ x_1,\ldots, x_\numperset \}$. 

This departs from the standard active learning for ranking problem, in which the true labels $Y$ for $X_q$ are revealed with certainty. Instead, the researcher observes noisy reflections of underlying utilities in the form of partial rank orderings. Facing uncertainty, each $x_i$ can be re-sampled to observe another realization of $Y_i$.
...
We let $l(a,y)$ be the loss by performing decision $a$ when true output is $y$. But since we are uncertain about $y$, we consider $E_{y \sim P(y|x,D)} \left[ l(a,y) \right]$, 
\begin{align}
\rho(a) = E_{y} \left[ l(a,y) \right] := \int_y l(a,y) P(y|x,D) dy .
\end{align}

The objective of the decision problem is to select the best $a$ to minimize expected loss based on training data $D$ considering some potential input data $x$,
\begin{align}
a^{*} := \text{arg}\min_a \rho(a) = \text{arg}\min_a \int_y l(a,y) P(y|x,D) dy  \\
EL(x) := \rho(a^{*}) = \min_a \int_y l(a,y) P(y|x,D) dy , 
\end{align}
so the $EL(x)$ is the minimized expected loss.

This can be understood for the marketing research problem as follows. The $a$ is the act of assigning a rank ordering $\pi$ of items' utilities $u$. The past data $D$ is the set of items observed so far and the collection of the implied partial rank orderings of those items based on surveys of respondent, which reflect utilities $u$. The input $x$ is the set of \numperset items considered to be asked to the next respondent, and the potential output $y$ is the potential collection of partial rank orderings of those items in input $x$.

We note that despite $a^{*}$ minimizing expected loss under the posterior, the $EL(x)$ is defined given an input $x$. For different possible choices of $x$, the best action $a^{*}$ may change, and therefore, the level of best loss $EL(x)$ may change too. 

Next we consider the possible choices of input $x$, the next data selected. 

Since we have access to a pool of possible items, $x \in \mathcal{X}$, we also know the distribution of those input values, $P(x)$. We want to select the next $x$that will minimize the generalization error

The expected error on unseen examples is the average expected loss over the input distribution, which is known as the generalization error $\int_x EL(x)P(x)dx$. We aim to minimize that generalized error, for any general input $x$.

Active learning strategy selects the input instance $x$ to maximize the expected loss $EL(x)$ given the input $x$: $\text{arg}\max_x EL(x)$. This is the essence of the ELO principle: To minimize generalized loss, select data that maximizes expected loss, even when taking the action that minimizes loss given that input and action.


For binary classification, intuitively, an active learning strategy would select the next data points that are likely to have predicted a class with probability near 0.5 since observing its true class will be most informative. Formally, the binary classification output is $y \in \mathcal{Y} = { -1, +1 } $ and the action is the prediction $a$ also in $\mathcal{Y}$. The loss is also binary, $l(a,y)=0$ if $a=y$, and $1$ if $a \neq y$. The loss-minimizing prediction is to select whichever of the two potential outcomes is more likely.
\begin{align}
a^{*} 
= \text{arg} \min_{a} P(y=a|x,D) 
= \text{arg} \min_{a} \left\{ 0 \cdot P(y=a|x,D) + 1 \cdot P(y \neq a|x,D) \right\} 
\end{align}

The minimized expected loss is then
\begin{align}
EL(x) = \min \left( P(y=1|x,D), P(y=0|x,D) \right).
\end{align}

The active learning strategies will select an $x$ to that would currently have the highest EL(x). In this case, 
\begin{align}
x^{*}  = \text{arg}\max_{x} \min \left( P(y=1|x,D), P(y=0|x,D) \right),
\end{align}
It selects the inputs $x$ where the predicted probabilities are at 0.5 $= P(y=1|x,D), P(y=0|x,D)$.
...
decision $\pi$ vs truth $\pi^{*}$


ELO principle for active learning says to select the $X_q$ with the highest expected losses, EL(q),
\begin{align}
EL(q) := \min_{a} \int_{Y} l(a,Y) P(Y|X_q,D) dY \\
q^{*} = \text{arg}\max_q EL(q) \\
EL^{*}(q) = \max_q EL(q) 
\end{align}

...
\label{table:nice500}
\end{table}
To test to see if these results would generalize to other data sets we tested on two other types of data sets, The first, labeled the uncorrelated data set each respondants preference utility are sampled from a uniform distribution. So each respondant's preferences are uncorrelated with every other respondant (the worst possible enviroment). We take the top $k$ highest mean prefence scores across the respondants to be the top $k$ true items. The results are 

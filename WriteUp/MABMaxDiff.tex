...
This can be understood for the  marketing research problem as follows. 


For the ranking and selection problem, the optimal action $a^{*}$ is the $\pi$ that 

The expected error on unseen examples is the {average expected loss over the input distribution $\int_x EL(x)P(x)dx$. To minimize generalized error, active learning selects the input instance $x$ to maximize the expected loss $EL(x)$:
select $\text{arg}\max_x EL(x)$.



The test points $X_q = (x_1,...,x_q)$ with unknown labels. 

$l(a,Y) = $ loss based on ranking $a$ if the true labels are $Y$.
\begin{align}
\int_Y l(a,Y) P(Y|X_q,D)dY
\end{align}

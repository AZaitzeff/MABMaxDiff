...
\TITLE{Active Learning for Choice Experiments: \\ Applications to Large-scale Adaptive MaxDiff \\
and Idea Screening \\
...
% Fill in data.
\KEYWORDS{idea screening, maximum-difference surveys, adaptive conjoint, active learning, multi-armed bandit, Bayesian bootstrap, best-worst scaling, Thompson Sampling, uncertainty minimization}
...
We also consider the practical implementation of the algorithms. The scalability and speed of each is considered. We provide alternative versions that are faster but may sacrifice theoretical statistical validity; nevertheless, some perform empirically well. 
...
We contribute three ideas to the literature and practice. First, we frame choice task data collection as an active learning problem, but we borrow ideas from an learning-and-earning problem. During the process of data collection, we simultaneously balance the desire to learn preferences of all items and the objective is to identify the best set of items. The faster we identify the best items, the more we include them in questions, improving the precision of estimates. And if we want to reach a desired level of precision, we can do so faster, saving money in the form of number of respondents and time. 

Learning-and-earning problems are also known as multi-armed bandit problems. MAB problems appear in marketing experiments for online advertising~\cite{schwartz2015customer},~\cite{urban2013morphing} and website design~\cite{hauser2009website}. One particular MAB method amenable to multi-variable parametric models is Thompson Sampling.\\
...
However, our problem is not a bandit problem exactly, but it is related and is a bandit-inspired problem. We find bandit algorithms serve as a good heuristic to achieve our learning objective better than existing MaxDiff and conjoint methods. To see how a pure bandit solution is limited, consider what would occur if the reward were whether an item is selected by a respondent as the best alternative. Then arm's reward would be completely dependent on all of the other items presented in the set (e.g., was it presented alongside poor items). 

Instead, we consider this an active learning problem. Active learning has a specific meaning in the statistics and machine learning literature. 
Active learning has hardly appeared in marketing. One exception is Dzyabura et al. 2011. However, they do not consider active learning from the perspective of the manager, and instead consider active learning algorithms serve as decision making heuristics for individual consumers. 
...
\textbf{Is our goal to identify top items with maximum precision? Because we do not show any data that we do that or that is even important for problem (identify top items)}
...
\label{fig:illustrate_ts}
	\begin{center}
    \subfloat[Early]{{\includegraphics[width=.4\textwidth]{plots/beliefdistrBegin.pdf} }}%
    \qquad
    \subfloat[Late]{{\includegraphics[width=.4\textwidth]{plots/beliefdistrEnd.pdf} }}%
    \end{center}
\end{figure}
...
\begin{figure}
\caption{ Illustration of \ts v \edts for two items at two points in time. \eric{Show a 4 plots in a 2x2 in this figure. Show the \fixedexpress, \ts, \edts belief distributions and show the underlying diffuse distribution that is one component of \edts. Show this for 1st ranked item (top row of plots) and 60th ranked item (bottom row) to show . Show for early beliefs after respondent 40 (left column of plots) and late beliefs after respondent 500 (right) show learning.} }
\label{fig:illustrating_edts}
 	\begin{center}
    \subfloat[Early, 1st rank]{{\includegraphics[width=.4\textwidth]{plots/beliefdistrBegin.pdf} }}%
    \qquad
    \subfloat[Late, 1st rank]{{\includegraphics[width=.4\textwidth]{plots/beliefdistrEnd.pdf} }}%
    \qquad
    \subfloat[Early, 60th rank]{{\includegraphics[width=.4\textwidth]{plots/endbetadis30.pdf} }}%
    \qquad
    \subfloat[Late, 60th rank]{{\includegraphics[width=.4\textwidth]{plots/endbetadis90.pdf} }}%
    \end{center}
\end{figure}
...
\eric{What range did we actually test? Where did we get $\epsilon = 1/5$? Did we test $\delta < 1/4$ ever?}
...
\label{fig:effects_epsilon_delta}
 	\begin{center}
    \subfloat[$\epsilon=1/2$]{{\includegraphics[width=.4\textwidth]{plots/10hitrate120show3.pdf} }}%
    \qquad
    \subfloat[$\epsilon=1/4$]{{\includegraphics[width=.4\textwidth]{plots/10hitrate120show3.pdf} }}%
    \end{center}
\end{figure}
...
\begin{figure}%
    \caption{Hit Rates for Top $k=\{3,10\}$ with 120 items. The cumulative hit rate obtained (y-axis) improves with the number of respondents interviewed (x-axis), but it does so at different rates for each algorithm. \eric{Please make y-axes the same across all k for the K120 L20 problem. }}%
...
\begin{figure}%
    \caption{Hit Rates for Top $k=\{20,40\}$ with 120 items.\eric{Please make y-axes the same across all k for the K120 L20 problem.}}%

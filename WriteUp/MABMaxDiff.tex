...
From~\cite{toubia2007adaptive}, after calculating the scores for a respondent, you perturb the scores by a normal vector with mean zero and variance $c\frac{1}{n_i}$, Where $n_i$ is the number of questions that has had item $i$ and $c$ is some constant, we take it to be $\frac{1}{10}$. Using the pertubed scores Serve up the $L$ items accordingly.\\
...
\textbf{Top k hit rate}: What percent of the top k true items the estimated scores using robotic respondents identified. For instance, if k is 10 and if the estimated scores identified 7 of the true top 10 items (irrespective of order), the hit rate was 70\%. We use $k=3,10,20,40$.
...
Using bootstrap sampling (sampling with replacement), we simulated the process of collecting respondent data up to sample sizes of $N=500$.  We considered the first 20 respondents to be the initial group, and all adaptive methods only begin after the 20th respondent. Each robotic respondent completed 12 choice sets, where each set included 5 items ($L=20$, $J=12$, $|S|=5$).\\
As described earlier, we tested the different adaptive Maxdiff approaches found in table \ref{methods}, with ($\epsilon=\frac{1}{4}$, $\delta=\frac{1}{4}$) for $\epsilon$-diffuse methods, against an existing fixed MaxDiff approach:\\
...
\label{table:at500}
\end{table}
Figures \ref{fig:3hit}, \ref{fig:10hit}, \ref{fig:20hit}, and \ref{fig:40hit} and tables \ref{table:at260} and \ref{table:at500} show the results for the 120-item dataset.  The X-axis indicates the number of cumulative respondents interviewed, and the Y-axis reports the hit rate obtained at each cumulative sample size.  For example, after the first 200 respondents, the Fixed Express Design obtains a top 3 hit rate of 60\% whereas the other approaches achieve a top 3 hit rate of about 80\%.\\
The key takeaway from the data is simple: a good adaptive method is at least 3x more efficient than the standard Fixed Express MaxDiff approach under any value of $k$. The good adaptive methods being Greatest Uncertainty with random perturbations, Misclassification Minimization with random perturbations, $\epsilon$-diffuse Thompson sampling with thresholding, and $\epsilon$-greedy with thresholding under any choice of $k$ and $\epsilon$-diffuse Thompson Sampling  when $k<L$ and $\epsilon$-greedy when $k<<L$. 
...
Almost all of the adaptive methods have parameters. Greatest Uncertainty with random perturbation and Misclassification Minimization with random perturbation $c=\infty$ (or any $c \geq 250$ in the case 120 items showing 20 items to each respondent), $\epsilon$-greedy with $\epsilon=1$, and $\epsilon$-diffuse TS with $\epsilon=1$ and $\delta=0$ are the same sampling scheme as fixed express. Also $\epsilon$-diffuse TS with $\epsilon=0$ is regular TS. The trade-off is robustness against bad early respondents verses speed of convergence. We recommend $\frac{1}{5}\leq \epsilon \leq \frac{1}{2}$, $\frac{1}{4}\leq \delta \leq \frac{1}{2}$ and $.05 \leq c \leq 1$.
%I personally like c=.5

...
\label{fig:3util}
\end{figure}
In figure \ref{fig:3util} we see after 200 respondents TS on average is about .97 so from table \ref{table:PTU} we can deduce that usually TS has the top 2 items in the top 3 and trying to decide between the 3rd and 4th item. After 200 respondents Sparse Express is about .89 which means that on average after 200 iterations has top 5 items in its top 3. Compare that to before, where Sparse Express a 3 hit rate score of 0.58. Combined we know that Sparse Expressed finds which the tops items but has trouble distinguishing what is the correct ranking of those items.
\subsection{Using Regret as a Stopping Rule}
From figure \ref{fig:3util} after 600 respondents TS finds the top 3 items most of the time. Thus it becomes inefficient to keep giving surveys after this point. In practice we do not know the True Utilities \textit{a priori} but we would still like to know when we should stop.\\
